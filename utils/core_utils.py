...
def load_config_and_checkpoint(config_path, default_root_dir):
    ...
    if checkpoint:
        ...
        return (
            torch.load(checkpoint, map_location=torch.device("cpu"), weights_only=False)["hyper_parameters"],
            checkpoint,
        )
    else:
...
def get_trainer(config: dict, default_root_dir: str | None, fast_dev_run=False):
    ...
    if isinstance(default_root_dir, str) and find_latest_checkpoint(default_root_dir) is not None:
        logging.info(
    ...
    filename_suffix = (
        ...
        if (hasattr(logger, "experiment") and hasattr(logger.experiment, "id") and logger.experiment.id is not None)
        else ""
    )
...
def get_stage_module(config, stage_module_class, checkpoint_path=None, checkpoint_resume_dir=None):
    # get a default_root_dr
    default_root_dir = get_default_root_dir(config['stage_dir'])
    ...
    if checkpoint_resume_dir is not None:
        if not os.path.exists(checkpoint_resume_dir):
            raise Exception(f"Checkpoint resume directory {checkpoint_resume_dir} does not exist.")
        if not find_latest_checkpoint(checkpoint_resume_dir, "*.ckpt"):
            raise Exception("No checkpoint found in checkpoint resume directory" f" {checkpoint_resume_dir}.")
    ...
    if default_root_dir is not None and find_latest_checkpoint(default_root_dir, "*.ckpt"):
